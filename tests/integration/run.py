...
class Qualification:
    def __init__(self, args, directory):
        ...
        self.kubeconfig = (
            str(Path(args.kubeconfig).resolve()) if args.kubeconfig else str(self.directory / "kubeconfig")
        )
            ...
            "stores": {
                "mongo": {
                    ...
                    "worker": {
                        ...
                        "autoscaling": {"allowScaleToZero": True, "keda": {"kafkaLag": {"targetLag": 100}}},
                    },
                    "kafka": {
                        "brokers": ["kafka:9092"],
                        "topic": {"name": "mongo-mutations"},
                        "topicPolicy": {"partitions": 4, "replicationFactor": 1, "minInSyncReplicas": 1},
                    },
                }
            },
            "defaults": {
        ...
        if args.sink_image:
            self.values["image"] = {
                "repository": args.sink_image.rsplit(":", 1)[0],
                "tag": args.sink_image.rsplit(":", 1)[1],
                "digest": "",
            }
        if args.scenario != "upgrades":
            ...
            for settings in [self.values["gateway"], *self.values["defaults"].values()]:
                settings["config"] = {"logging": copy.deepcopy(logging)}
        self.events = []
        self.owned = False
    ...
    def get(self, kind, name=None):
        return json.loads(
            self.command(self.kubectl + ["get", kind] + ([name] if name else []) + ["-o", "json"])
        )
        ...
        while time.monotonic() < deadline:
            if self.traffic_active:
                ...
                if phase in ("Failed", "Succeeded"):
                    raise RuntimeError(
                        "continuous traffic stopped before the scenario completed: "
    ...
    def probe_result(self, name):
        self.wait(
            f"business probe {name} completes",
            lambda: self.get("pod", name)["status"].get("phase") in ("Succeeded", "Failed"),
        )
    ...
    def setup(self):
        ...
        corefile, replacements = re.subn(
            r"(?m)^(\s*)ttl\s+\d+$", r"\g<1>ttl 30", dns_config["data"]["Corefile"]
        )
        if replacements != 1:
        ...
        cached = []
        for image in [
            self.args.sink_image or UPGRADE_IMAGE,
            self.args.upgrade_image,
            "mongo:8.2",
            "apache/kafka:4.2.1",
        ...
        self.command(
            ["go", "build", "-trimpath", "-o", str(self.directory / "probe"), "."],
            cwd=ROOT / "tests/integration/probe",
            env=environment,
        )
        ...
        self.wait(
            "isolated Mongo and Kafka ready", lambda: self.available("mongo") and self.available("kafka")
        )
        self.command(
            self.kubectl
            + [
        ...
        for version in [1, 2]:
            ...
            secret = {
                "apiVersion": "v1",
                "kind": "Secret",
                "metadata": {"name": f"mongo-v{version}"},
                "immutable": True,
                "stringData": {"uri": uri},
            }
            self.apply([secret])
    ...
    def exercise(self):
        ...
        self.wait(
            "persistent business client starts",
            lambda: self.get("pod", "traffic")["status"].get("phase") == "Running",
        )
        time.sleep(10)
        self.log("rolling Gateway and Engine with default drain budgets")
        self.values["gateway"]["rollout"] = {"restartRevision": "roll-1"}
        # Keep a Worker active so both roles rotate under business traffic.
        self.values["stores"]["mongo"]["worker"]["replicas"] = 1
        self.upgrade()
        ...
        self.wait(
            "invalid rotation reference stalls new Pods while old traffic continues",
            lambda: any(
                ...
                for event in self.get("events")["items"]
            ),
            timeout=120,
        )
        ...
        self.values["stores"]["mongo"]["worker"]["replicas"] = 0
        self.upgrade()
        self.wait("rotation Worker drains", self.no_terminating)
        self.log("Engine scale 2 -> 3 -> 1 -> 2 under traffic")
        self.values["stores"]["mongo"]["engine"]["replicas"] = 3
        self.upgrade()
        self.wait(
            "third Engine becomes discoverable before scale-down",
            lambda: self.available("qual-sink-mongo-engine"),
        )
        self.values["stores"]["mongo"]["engine"]["replicas"] = 1
        self.values["stores"]["mongo"]["engine"]["podDisruptionBudget"] = {"maxUnavailable": 0}
        self.upgrade()
        self.wait("single Engine remains after scale-down", self.no_terminating)
        self.values["stores"]["mongo"]["engine"]["replicas"] = 2
        self.values["stores"]["mongo"]["engine"]["podDisruptionBudget"] = {"maxUnavailable": 1}
        self.upgrade()
        self.wait("second Engine becomes discoverable", lambda: self.available("qual-sink-mongo-engine"))
        invalid = copy.deepcopy(self.values)
        invalid["stores"]["archive"] = {"phase": "active", "storage": self.storage("mongo-v2")}
        self.upgrade(invalid, "must first be staged")
        self.values["stores"]["archive"] = {
            "phase": "staged",
            "storage": self.storage("mongo-v2"),
            "engine": {},
        }
        for reference, expected in [
            ({"name": "absent-secret", "key": "uri"}, 'secret "absent-secret" not found'),
            ...
            self.wait(
                "missing Secret/key blocks staged Engine startup",
                lambda: any(
                    expected in event.get("message", "")
                    and event.get("involvedObject", {})
                    .get("name", "")
                    .startswith("qual-sink-archive-engine-")
                    for event in self.get("events")["items"]
                ),
                timeout=120,
            )
            invalid = copy.deepcopy(self.values)
            invalid["stores"]["archive"]["phase"] = "active"
            self.upgrade(invalid, "Engine is not fully available")
        self.values["stores"]["archive"]["storage"] = self.storage("mongo-v2")
        self.upgrade()
        invalid = copy.deepcopy(self.values)
        invalid["stores"]["archive"]["phase"] = "active"
        invalid["stores"]["archive"]["engine"]["rollout"] = {"restartRevision": "unverified"}
        ...
        if not self.available("qual-sink-archive-engine"):
            self.upgrade(invalid, "Engine is not fully available")
        self.wait(
            "staged Engine passes the discovery availability window",
            lambda: self.available("qual-sink-archive-engine"),
        )
        self.command(self.kubectl + ["scale", "deployment/qual-sink-archive-engine", "--replicas=1"])
        self.wait(
            "staged Engine manual under-scaling is observed",
            lambda: self.available("qual-sink-archive-engine"),
        )
        self.upgrade(invalid, "Engine is not fully available")
        self.command(self.kubectl + ["scale", "deployment/qual-sink-archive-engine", "--replicas=2"])
        self.wait(
            "staged Engine returns to the configured minimum",
            lambda: self.available("qual-sink-archive-engine"),
        )
        self.values["stores"]["archive"]["phase"] = "active"
        self.upgrade()
        ...
        self.values["stores"]["archive"]["phase"] = "retiring"
        self.upgrade()
        invalid = copy.deepcopy(self.values)
        invalid["stores"]["archive"]["phase"] = "staged"
        self.upgrade(invalid, "cannot return to staged")
        invalid = copy.deepcopy(self.values)
        del invalid["stores"]["archive"]
        self.upgrade(invalid, "removal requires")
        invalid["cluster"] = {"storeLifecycle": {"removalApprovals": ["archive"]}}
    ...
    def exercise_upgrades(self):
        old_image = self.get("deployment", "qual-sink-gateway")["spec"]["template"]["spec"]["containers"][0][
            "image"
        ]
        new_image = self.args.upgrade_image
        if old_image == new_image:
            raise RuntimeError(
                "rollout qualification requires two distinct image references from compatible builds"
            )
        repository, tag = new_image.rsplit(":", 1)
        target = {"repository": repository, "tag": tag, "digest": ""}
        baseline = copy.deepcopy(self.values["image"])
        self.values["stores"]["mongo"]["worker"]["replicas"] = 1
        self.upgrade()
        ...
        self.traffic_active = True
        self.wait(
            "persistent version-rollout client starts",
            lambda: self.get("pod", "traffic")["status"].get("phase") == "Running",
        )
        self.log(f"qualifying shared image {old_image} -> {new_image} -> {old_image}")
        stages = [("image-upgrade", target, new_image), ("image-rollback", baseline, old_image)]
        evidence = []
        for stage, image, expected_image in stages:
            self.values["image"] = copy.deepcopy(image)
            self.log(stage)
            self.upgrade()
            ...
            expected = {name: expected_image for name in actual}
            if actual != expected:
                raise RuntimeError(f"{stage}: wrong shared image: {actual}, expected {expected}")
            ...
            self.probe(
                f"{stage}-publish", ["-mode", "publish", "-dataset", f"{stage}-async", "-count", "100"]
            )
        ...
        self.log(f"shared image upgrade and rollback reconciled; continuous records={continuous['verified']}")
    ...
    def exercise_scaling(self):
        ...
        worker["autoscaling"].update(scaling)
        self.upgrade(wait=False)
        self.wait(
            "KEDA scales backlog to multiple consumers",
            lambda: self.get("deployment", "qual-sink-mongo-worker")["spec"]["replicas"] >= 2,
            timeout=180,
        )
        self.command(
            self.kubectl + ["label", "node", f"{self.cluster}-control-plane", "sink-test-workers=ready"]
        )
        self.wait(
            "multiple Worker Pods become ready",
            lambda: self.get("deployment", "qual-sink-mongo-worker").get("status", {}).get("readyReplicas", 0)
            >= 2,
            timeout=180,
        )
        self.wait("multiple Kafka consumers receive partitions", self.consumers_assigned, timeout=90)
        self.probe(
            "verify", ["-mode", "verify", "-dataset", "async", "-count", str(published["acknowledged"])]
        )
        verified = self.probe_result("verify")
        self.wait(
            "KEDA returns Workers to zero after lag drains",
            lambda: self.get("deployment", "qual-sink-mongo-worker")["spec"]["replicas"] == 0,
            timeout=600,
        )
    ...
    def consumers_assigned(self):
        ...
        return (
            sum(
                1
                for row in rows
                if len(row) >= 5 and row[0] == "mongo-workers" and row[4].isdigit() and int(row[4]) > 0
            )
            >= 2
        )
...
def main():
    ...
    parser.add_argument(
        "--sink-image", help="explicit local candidate repository:tag; loaded into the owned cluster"
    )
    parser.add_argument(
        "--scenario",
        choices=["all", "scaling", "upgrades"],
        default="all",
        help="run deployment, Kafka/KEDA, or mixed-version qualification",
    )
    parser.add_argument(
        "--upgrade-image", default=UPGRADE_IMAGE, help="target repository:tag for the upgrades scenario"
    )
    parser.add_argument("--cluster", help="owned sink-chart-* name; defaults to a unique name")
    parser.add_argument(
        "--kubeconfig", help="explicit owned Kind kubeconfig for development reuse; cluster is still deleted"
    )
