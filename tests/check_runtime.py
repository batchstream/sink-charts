...
def main():
    ...
    parser.add_argument(
        "--sink-image", help="explicit candidate image; defaults to the chart's pinned release"
    )
    ...
    image = args.sink_image or (
        f"{defaults['repository']}@{defaults['digest']}"
        if defaults["digest"]
        else f"{defaults['repository']}:{defaults['tag']}"
    )
    examples = [
        ["cluster-values.yaml"],
        ["search-values.yaml"],
        ["cluster-values.yaml", "logging-values.yaml"],
        ["cluster-values.yaml", "tuning-values.yaml", "logging-values.yaml"],
    ]
